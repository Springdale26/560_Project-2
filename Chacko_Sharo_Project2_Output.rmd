...
library(caTools)
set.seed(1234)
split_ratio = sample.split(BreastCancer, SplitRatio = 0.8)
train = subset(BreastCancer, split_ratio==TRUE)
test = subset(BreastCancer, split_ratio==FALSE)
...
for (i in 1:length(BreastCancer[,1])) {
  ...
  mytree.predloo <- predict(mytree,BreastCancer[i,],type="class")
...
cm <- confusionMatrix(ans,BreastCancer$Class)
